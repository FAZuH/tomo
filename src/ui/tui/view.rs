        ...
        vec![
            ...
            S::section("󰔛 Pomodoro Timer", vec![
                ...
                S::subsection("Auto Start", vec![
                    S::checkbox("Focus", state.timer_auto_focus),
                    S::checkbox("Short Break", state.timer_auto_short),
                    S::checkbox("Long Break", state.timer_auto_long),
                ]),
            ]),
            S::section("󰛢 Command Hooks", vec![
                S::subsection("Hooks", vec![
                    S::input("Focus", &state.hook_focus),
                    S::input("Short Break", &state.hook_short),
                    S::input("Long Break", &state.hook_long),
                ]),
            ]),
            S::section("󰕾 Sounds", vec![
                S::subsection("Sound Files", vec![
                    S::input("Focus", state.sound_focus.as_ref().map(|p| p.display().to_string()).unwrap_or_default()),
                    S::input("Short Break", state.sound_short.as_ref().map(|p| p.display().to_string()).unwrap_or_default()),
                    S::input("Long Break", state.sound_long.as_ref().map(|p| p.display().to_string()).unwrap_or_default()),
                ]),
            ]),
        ]
    }
